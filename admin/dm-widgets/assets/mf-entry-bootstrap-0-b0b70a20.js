...
(async () => {
  const __mfHostInit = await __mfImport("./hostInit-DASV5BfL.js");
  await __mfHostInit.__tla;
  const { initHost } = __mfHostInit;
  await initHost();
  const __mfPendingShares = await __mfImport("./pendingShares-CfNQAJDo.js").catch(() => undefined);
...
})().then(() => __mfImport("./index-CPINTgm1.js"));
